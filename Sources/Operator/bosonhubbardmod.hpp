...
#ifndef NETKET_BOSONHUBBARD_MOD_HPP
#define NETKET_BOSONHUBBARD_MOD_HPP
...
class BoseHubbardMod : public AbstractOperator {
 ...
 public:
  ...
  BoseHubbardMod(std::shared_ptr<const AbstractHilbert> hilbert, double U,
              double V = 0., double G = 0., double mu = 0.)
    ...
    for (int i = 0; i < nsites_; i++) {
      ...
      for (auto bond : bonds_[i]) {
        ...
        if (v(i) > 0 && v(bond) < nmax_) {
          ...
          mel.push_back(-1.0*V_*std::sqrt(v(i)) * std::sqrt(v(bond) + 1));
        }
        if (v(bond) > 0 && v(i) < nmax_) {
          connectors.push_back(std::vector<int>({bond, i}));
          newconfs.push_back(std::vector<double>({v(bond) - 1, v(i) + 1}));
          mel.push_back(-1.0*V_*std::sqrt(v(bond)) * std::sqrt(v(i) + 1));
        }
      }
    }
    mel[0] += G_/nsites_ * d*d;
  }
};
